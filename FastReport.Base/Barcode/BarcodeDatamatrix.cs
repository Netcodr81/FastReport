...
namespace FastReport.Barcode
{
    ...
    public class BarcodeDatamatrix : Barcode2DBase
    {
        ...
        private const byte UPPER_CASE_SIGN = (byte)235;

        private const string SET_X12 = "\r*> 0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";

        const string SHIFTED_SET_C40_AND_TEXT = "!\"#$%&'()*+,-./:;<=>?@[\\]^_";
        const string BASE_SET_C40 = " 0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        const string SHIFTED_SET_C40 = "`abcdefghijklmnopqrstuvwxyz{|}~\u007f";
        const string BASE_SET_TEXT = " 0123456789abcdefghijklmnopqrstuvwxyz";
        const string SHIFTED_SET_TEXT = "`ABCDEFGHIJKLMNOPQRSTUVWXYZ{|}~\u007f";
        ...
        private static int AsciiEncodation(byte[] text, int textOffset, int textLength, byte[] data, int dataOffset, int dataLength, int symbolIndex)
        {
            ...
            {
                c = text[textIndex++] & 0xff;

                if (IsDigit(c) && symbolIndex < 0 && textIndex < textLength && IsDigit(text[textIndex] & 0xff))
                {
                    data[dataIndex++] = (byte)((c - '0') * 10 + (text[textIndex++] & 0xff) - '0' + 130);
                }
                else if (c == 232)
                {
                    data[dataIndex++] = (byte)232;
                }
                else if (c > 127)
                {
        ...
        private static int B256Encodation(byte[] text, int textOffset, int textLength, byte[] data, int dataOffset, int dataLength, int prevEnc, int origDataOffset)
        {
            ...
            if (textLength < 250)
            {
                ...
                minRequiredDataIncrement = prevEnc != (int)DatamatrixEncoding.Base256 ? 2 : 0;
            }
            else if (textLength == 250 && prevEnc == (int)DatamatrixEncoding.Base256)
            {
            ...
            else
            {
                data[simulatedDataOffset + 1] = (byte)(textLength / 250 + 249);
                data[simulatedDataOffset + 2] = (byte)(textLength % 250);
                minRequiredDataIncrement = prevEnc != (int)DatamatrixEncoding.Base256 ? 3 : 0;
            }
            ...
            for (int j = prevEnc != (int)DatamatrixEncoding.Base256 ? dataOffset + 1 : dataOffset; j < minRequiredDataIncrement + textLength + dataOffset; ++j)
            {
        ...
        private static int X12Encodation(byte[] text, int textOffset, int textLength, byte[] data, int dataOffset, int dataLength, int symbolIndex, int origDataOffset)
        {
            if (textLength == 0)
                return 0;
            ...
            {
                int i = SET_X12.IndexOf((char)text[textIndex + textOffset]);
                if (i >= 0)
                {
                    x[textIndex] = (byte)i;
                    ++count;
            ...
            for (; textIndex < textLength; ++textIndex)
            {
                ...
                if (c < 40)
                {
                    if (textIndex == 0 && latch || textIndex > 0 && x[textIndex - 1] > 40)
                        data[dataOffset + dataIndex++] = LATCH_X12;
                    if (dataIndex + 2 > dataLength)
                        break;
                    n = 1600 * x[textIndex] + 40 * x[textIndex + 1] + x[textIndex + 2] + 1;
                ...
                else
                {
                    if (symbolIndex <= 0)
                    {
                        if (textIndex > 0 && x[textIndex - 1] < 40)
                            data[dataOffset + dataIndex++] = UNLATCH;
                    }

                    int i = AsciiEncodation(text, textOffset + textIndex, 1, data, dataOffset + dataIndex, dataLength, -1);
                    if (i < 0)
                        return -1;
                    if (data[dataOffset + dataIndex] == UPPER_CASE_SIGN)
                        dataIndex++;
                    dataIndex++;
                }
            }
        ...
        private static int EdifactEncodation(byte[] text, int textOffset, int textLength, byte[] data, int dataOffset, int dataLength, int symbolIndex, int prevEnc, int origDataOffset, bool sizeFixed)
        {
            ...
            for (; textIndex < textLength; ++textIndex)
            {
                c = text[textIndex + textOffset] & 0xff;
                if (((c & 0xe0) == 0x40 || (c & 0xe0) == 0x20) && c != '_')
                {
                    if (ascii)
                    {
                        if (dataIndex + 1 > dataLength)
                            break;
                        data[dataOffset + dataIndex++] = LATCH_EDIFACT;
                        ascii = false;
                    }
                    c &= 0x3f;
                    edi |= c << pedi;
                ...
                else
                {
                    ...
                    if (IsDigit(c) && textOffset + textIndex > 0 && IsDigit(text[textOffset + textIndex - 1] & 0xff) &&
                            prevEnc == (int)DatamatrixEncoding.Edifact && data[dataOffset - 1] >= 49 && data[dataOffset - 1] <= 58)
                    {
                        data[dataOffset + dataIndex - 1] = (byte)(((text[textOffset - 1] & 0xff) - '0') * 10 + c - '0' + 130);
                        dataIndex--;
                    }
                    else
                    {
            ...
            if (dataSize - dataOffset - dataIndex <= 2 && pedi >= 6)
            {
                ...
                if (pedi <= 12)
                {
                    ...
                    if ((val & 0x20) == 0)
                        val |= 0x40;
                    data[dataOffset + dataIndex++] = (byte)(val + 1);
                }
        ...
        private static int C40OrTextEncodation(byte[] text, int textOffset, int textLength, byte[] data, int dataOffset, int dataLength, bool c40, int symbolIndex, int prevEnc, int origDataOffset)
        {
            ...
            int textIndex = 0;
            int dataIndex = 0;
            shift2 = SHIFTED_SET_C40_AND_TEXT;
            if (c40)
            {
                basic = BASE_SET_C40;
                shift3 = SHIFTED_SET_C40;
            }
            else
            {
                basic = BASE_SET_TEXT;
                shift3 = SHIFTED_SET_TEXT;
            }

            int mode = c40 ? (int)DatamatrixEncoding.C40 : (int)DatamatrixEncoding.Text;
            if (symbolIndex != -1)
                return AsciiEncodation(text, textOffset, 1, data, dataOffset, dataLength, prevEnc == mode ? 1 : -1);
            ...
            while (textIndex < textLength)
            {
                ...
                c = text[textOffset + textIndex++] & 0xff;
                if (c > 127)
                {
                    c -= 128;
            ...
            {
                i = AsciiEncodation(text, textOffset + textIndex, textLength - textIndex, data, dataOffset + dataIndex, dataLength - dataIndex, -1);
                return i + dataIndex + dataOffset - origDataOffset;
            }
            return dataIndex + dataOffset - origDataOffset;
        }

        private int GetEncodation(byte[] text, int textOffset, int textSize, byte[] data, int dataOffset, int dataSize, bool sizeFixed, bool firstMatch)
        {
            int e, j, k;
            int[] e1 = new int[6];
            if (dataSize < 0)
            ...
            {
                e1[0] = AsciiEncodation(text, textOffset, textSize, data, dataOffset, dataSize, -1);
                if (firstMatch && e1[0] >= 0)
                    return e1[0];
                e1[1] = C40OrTextEncodation(text, textOffset, textSize, data, dataOffset, dataSize, true, -1, -1, dataOffset);
                if (firstMatch && e1[1] >= 0)
                    return e1[1];
                e1[2] = C40OrTextEncodation(text, textOffset, textSize, data, dataOffset, dataSize, false, -1, -1, dataOffset);
                if (firstMatch && e1[2] >= 0)
                    return e1[2];
                e1[3] = B256Encodation(text, textOffset, textSize, data, dataOffset, dataSize, -1, dataOffset);
                if (firstMatch && e1[3] >= 0)
                    return e1[3];
                e1[4] = X12Encodation(text, textOffset, textSize, data, dataOffset, dataSize, -1, dataOffset);
                if (firstMatch && e1[4] >= 0)
                    return e1[4];
                e1[5] = EdifactEncodation(text, textOffset, textSize, data, dataOffset, dataSize, -1, -1, dataOffset, sizeFixed);
                ...
                if (j == 0)
                    e = AsciiEncodation(text, textOffset, textSize, data, dataOffset, dataSize, -1);
                else if (j == 1)
                    e = C40OrTextEncodation(text, textOffset, textSize, data, dataOffset, dataSize, true, -1, -1, dataOffset);
                else if (j == 2)
                    e = C40OrTextEncodation(text, textOffset, textSize, data, dataOffset, dataSize, false, -1, -1, dataOffset);
                else if (j == 3)
                    e = B256Encodation(text, textOffset, textSize, data, dataOffset, dataSize, -1, dataOffset);
                else if (j == 4)
                    e = X12Encodation(text, textOffset, textSize, data, dataOffset, dataSize, -1, dataOffset);
                return e;
            }
            ...
            switch (encoding)
            {
                case DatamatrixEncoding.Ascii:
                    return AsciiEncodation(text, textOffset, textSize, data, dataOffset, dataSize, -1);
                ...
                case DatamatrixEncoding.Base256:
                    return B256Encodation(text, textOffset, textSize, data, dataOffset, dataSize, -1, dataOffset);
                case DatamatrixEncoding.X12:
                    return X12Encodation(text, textOffset, textSize, data, dataOffset, dataSize, -1, dataOffset);
        ...
        private void Generate(byte[] text, int textOffset, int textSize)
        {
            ...
            if (height == 0 || width == 0)
            {
                ...
                e = GetEncodation(text, textOffset, textSize, data, extCount, last.dataSize - extCount, false, false);
                if (e < 0)
                {
            ...
            else
            {
                ...
                dm = dmSizes[k];
                e = GetEncodation(text, textOffset, textSize, data, extCount, dm.dataSize - extCount, true, true);
                if (e < 0)
                {
                    throw new Exception("The text is too big.");
                }
